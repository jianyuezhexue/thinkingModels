<template>
  <Page :description="isEdit ? '修改模型内容和设置' : '创建并分享你的思维模型'" :title="isEdit ? '编辑模型' : '创建思维模型'"
        ...
        <ElCard shadow="hover" class="!rounded-xl">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-4">
              <button v-for="(step, index) in steps" :key="step.id"
                class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all" :class="[
                  currentStep === index
                ...
                <div class="w-10 h-10 rounded-full flex items-center justify-center text-lg" :class="[
                  currentStep === index
          ...
          <ElForm ref="formRef" :model="form" :rules="rules" label-position="top" class="max-w-2xl">
            ...
            <ElFormItem label="模型封面">
              <div class="w-full space-y-3">
                <!-- 已选封面预览 -->
                <div v-if="form.cover" class="relative w-full h-48 rounded-xl overflow-hidden group">
                ...
                <div v-else class="w-full">
                  ...
                  <div
                    ...
                    <p class="text-xs text-purple-400 mt-1">高质量免费图片可选</p>
                  </div>
                </div>
              </div>
            </ElFormItem>
            ...
            <ElFormItem label="视频地址">
              <ElInput v-model="form.videoUrl" placeholder="输入视频链接（可选），支持 YouTube、Bilibili 等平台" maxlength="500"
                show-word-limit class="!rounded-lg">
                <template #prefix>
            ...
            <ElFormItem label="模型名称" prop="title">
              <ElInput v-model="form.title" placeholder="给你的思维模型起个名字，例如：SWOT 分析模型" maxlength="50" show-word-limit
                class="!rounded-lg" />
            </ElFormItem>

            <!-- 模型描述 -->
            <ElFormItem label="模型描述" prop="description">
              <ElInput v-model="form.description" type="textarea" :rows="4" placeholder="简要描述这个思维模型的用途、适用场景和价值..."
            ...
            <ElFormItem prop="category">
              ...
              <div v-else class="grid grid-cols-4 gap-3">
                <button v-for="cat in categories" :key="cat.value" type="button"
                  class="p-3 rounded-lg border-2 text-center transition-all" :class="[
            ...
            <ElFormItem label="标签">
              <div class="space-y-3">
                <div class="flex flex-wrap gap-2">
                  <ElTag v-for="tag in form.tags" :key="tag" closable effect="plain"
                    class="!bg-purple-50 !text-purple-600 !border-purple-200 !rounded-full"
                    @close="handleRemoveTag(tag)">
                    {{ tag }}
                  </ElTag>
                  <ElInput v-if="tagInputVisible" v-model="tagInput" size="small" class="!w-24"
                    @keyup.enter="handleAddTag" @blur="handleAddTag" />
                  <ElButton v-else-if="form.tags.length < 5" size="small" class="!rounded-full"
                ...
                <div class="flex flex-wrap gap-2">
                  ...
                  <button v-for="tag in suggestedTags.filter(t => !form.tags.includes(t))" :key="tag" type="button"
        ...
        <template v-if="currentStep === 1">
          ...
          <ElCard shadow="hover" class="!rounded-xl">
            <template #header>
              <div class="flex items-center gap-2">
                <span class="text-lg">📖</span>
                <span class="font-semibold text-gray-700">模型内容</span>
              </div>
            </template>
            <p class="text-sm text-gray-500 mb-4">介绍这个思维模型的背景、原理和核心价值</p>
            <ElInput v-model="form.content" type="textarea" :rows="6" placeholder="详细介绍这个思维模型的背景、理论基础、核心原理和适用场景..." />
          </ElCard>
          ...
          <ElCard shadow="hover" class="!rounded-xl">
            ...
            <div class="space-y-4">
              <div v-for="(step, index) in form.usageGuide" :key="index"
                ...
                <div class="flex items-start gap-4">
                  ...
                  <div class="flex-1 space-y-3">
                    <ElInput v-model="step.title" placeholder="步骤标题，例如：识别优势 (Strengths)" />
                    <ElInput v-model="step.description" type="textarea" :rows="3" placeholder="详细说明这个步骤的操作方法和注意事项..." />
                  </div>
                  <ElButton v-if="form.usageGuide.length > 1" type="danger" plain size="small" class="!rounded-full"
          ...
          <ElCard shadow="hover" class="!rounded-xl">
            ...
            <div class="space-y-4">
              <div v-for="(example, index) in form.examples" :key="index"
                class="p-4 rounded-xl bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-100">
                <div class="flex items-start gap-4">
                  <div
                    class="w-10 h-10 rounded-full bg-amber-500 text-white flex items-center justify-center font-bold flex-shrink-0">
                    {{ index + 1 }}
                  </div>
                  <div class="flex-1 space-y-3">
                    <ElInput v-model="example.title" placeholder="案例标题，例如：某电商平台的 SWOT 分析" />
                    <ElInput v-model="example.content" type="textarea" :rows="4" placeholder="详细描述这个案例的背景、分析过程和结论..." />
                  </div>
                  <ElButton v-if="form.examples.length > 1" type="danger" plain size="small" class="!rounded-full"
          ...
          <div class="max-w-xl space-y-6">
            ...
            <div class="p-6 rounded-xl bg-gradient-to-r from-purple-50 to-indigo-50 border border-purple-100">
              ...
              <div class="flex items-center gap-4 mb-4">
                <button type="button" class="flex-1 p-4 rounded-xl border-2 transition-all text-center"
              ...
              <div v-if="!form.isFree" class="space-y-4">
                <div>
                  ...
                  <div class="flex items-center gap-3">
                    <ElInput v-model.number="form.price" type="number" min="1" max="999" class="!w-32">
                      <template #prefix>¥</template>
                    </ElInput>
                    <div class="flex gap-2">
                      <button v-for="price in [9, 19, 29, 49, 99]" :key="price" type="button"
        ...
        <ElCard shadow="hover" class="!rounded-xl">
          <div class="flex items-center justify-between">
            ...
            <div class="flex items-center gap-3">
              ...
              <ElButton v-if="currentStep < 2" type="primary"
                class="!bg-purple-600 !border-purple-600 hover:!bg-purple-700 !rounded-full" @click="nextStep">
                下一步
              ...
              <ElButton v-else type="primary"
                class="!bg-purple-600 !border-purple-600 hover:!bg-purple-700 !rounded-full" @click="handleSubmit"
        ...
        <ElCard shadow="hover" class="!rounded-xl">
          ...
          <div class="space-y-4">
            ...
            <div class="flex flex-wrap gap-1">
              <span v-for="tag in form.tags.slice(0, 3)" :key="tag"
            ...
            <div class="flex items-center justify-between pt-3 border-t border-gray-100">
              <span class="text-sm text-gray-500">
                {{form.category.length > 0 ? form.category.map(v => categories.find(c => c.value ===
                  v)?.label).filter(Boolean).join('、') : '未选择分类'}}
              </span>
              <span :class="[
        ...
        <ElCard shadow="hover" class="!rounded-xl !bg-gradient-to-br from-purple-50 to-indigo-50 !border-purple-100">
          ...
          <div class="space-y-4">
            <div class="flex items-start gap-3">
              <div
                class="w-8 h-8 rounded-full bg-purple-200 text-purple-700 flex items-center justify-center font-bold text-sm flex-shrink-0">
                1</div>
              <div>
    ...
    <!-- 封面图片选择对话框 -->
    <ElDialog v-model="coverDialogVisible" title="选择预设封面" width="900" destroy-on-close :close-on-click-modal="true">
      <div class="space-y-4">
        ...
        <div class="grid grid-cols-4 gap-3 max-h-[480px] overflow-y-auto pr-2">
          <div v-for="cover in presetCovers" :key="cover.id"
            ...
            :class="{ 'ring-purple-500': form.cover === cover.url }" @click="selectPresetCover(cover.url)">
            <img :src="cover.url" :alt="cover.label" class="w-full h-full object-cover" loading="lazy" />
            <div
              class="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent flex items-end p-2 opacity-0 group-hover:opacity-100 transition-opacity">
              <span class="text-white text-xs font-medium">{{ cover.label }}</span>
            </div>
            <div v-if="form.cover === cover.url"
...
const form = reactive({
  title: '',
  description: '',
  category: [] as string[],  // 分类选择（单选，但用数组存储便于UI交互）
  tags: [] as string[],
  cover: '',
  videoUrl: '',
  isFree: true,
...
function toggleCategory(value: string) {
  // 单选：如果已选中则取消，否则选中
  form.category = form.category[0] === value ? [] : [value];
}
...
// 解析 JSON 数组，失败返回默认值
function parseJsonArray<T extends unknown[]>(jsonStr: string, defaultVal: T): T {
...
onMounted(async () => {
  ...
  if (isEdit.value && editId.value) {
    ...
    try {
      ...
      // 从独立字段读取 usageGuide 和 examples
      form.usageGuide = parseJsonArray(res.usageGuide, [{ title: '', description: '' }]);
      form.examples = parseJsonArray(res.examples, [{ title: '', content: '' }]);
...
// 步骤操作
function addStep() {
  form.usageGuide.push({ title: '', description: '' });
}

function removeStep(index: number) {
  if (form.usageGuide.length > 1) {
    form.usageGuide.splice(index, 1);
  }
}

// 案例操作
function addExample() {
  form.examples.push({ title: '', content: '' });
}
...
async function goToStep(step: number) {
  ...
  if (currentStep.value === 1) {
    if (!form.content.trim()) {
      ElMessage.warning('请填写模型内容');
      return;
    }
    const hasValidStep = form.usageGuide.some(s => s.title.trim() && s.description.trim());
    if (!hasValidStep) {
...
async function handleSubmit() {
  try {
    ...
    const usageGuide = form.usageGuide.filter((s: any) => s.title.trim() || s.description.trim());
    const examples = form.examples.filter((e: any) => e.title.trim() || e.content.trim());
    ...
    const submitData = {
      ...
      content: form.content,  // 模型内容直接存储
      usageGuide: JSON.stringify(usageGuide),   // 操作步骤单独存储
      examples: JSON.stringify(examples),   // 案例单独存储
      tags: form.tags.join(','),  // 标签转逗号分隔字符串
...
const formProgress = computed(() => {
  ...
  if (form.content) filled++;
  if (form.usageGuide.some(s => s.title && s.description)) filled++;
  if (form.examples.some(e => e.title && e.content)) filled++;
